  ...
  return (
    <Card
      className={`w-full mx-auto ${
        isMobile ? "max-w-xs bg-white/15" : "max-w-xs bg-white/10"
      } backdrop-blur-sm border-${
        isDark ? "gray-700/30" : "gray-300/30"
      } shadow-lg hover:${isDark ? "bg-gray-900/40" : "bg-white/20"} transition-all duration-300 z-5`}
    >
      <CardContent className={isMobile ? "p-2" : "p-3"}>
        <div className="flex items-center gap-3">
          <div
            className={`rounded-md overflow-hidden flex-shrink-0 bg-gradient-to-br from-blue-500/80 to-purple-600/80 ${
        ...
        <div className={isMobile ? "mt-2" : "mt-3"}>
          ...
          <div className={`flex items-center justify-center gap-3 ${isMobile ? "mt-1" : "mt-2"}`}>
            <Button
              variant="ghost"
              size="icon"
            ...
            >
              <SkipBack size={isMobile ? 10 : 14} />
            </Button>

            <Button
              onClick={togglePlay}
              className={`rounded-full ${isMobile ? "w-6 h-6" : "w-7 h-7"} ${
                isDark ? "bg-white/90 text-gray-900 hover:bg-white" : "bg-gray-900/90 text-white hover:bg-gray-900"
              } shadow-md`}
            >
              {isPlaying ? <Pause size={isMobile ? 10 : 14} /> : <Play size={isMobile ? 10 : 14} className="ml-0.5" />}
            </Button>
            ...
            >
              <SkipForward size={isMobile ? 10 : 14} />
            </Button>
          </div>
